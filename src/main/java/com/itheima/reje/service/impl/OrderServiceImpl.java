...
import com.itheima.reje.entity.*;
import com.itheima.reje.mapper.OrderMapper;
import com.itheima.reje.service.*;
...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Orders> implements OrderService {
    ...
    public void submit(Orders orders) {
        ...
        //用于计算订单总金额
        //原子操作，保证在多线程下也不会出现计算错误，保证线程安全
        AtomicInteger amount = new AtomicInteger(0);

        //遍历订单中的信息生成订单明细，并计算总金额
        List<OrderDetail> orderDetails = shoppingCarts.stream().map((item) -> {
            ...
            orderDetail.setAmount(item.getAmount());  //菜品或者套餐单份金额

            //计算总金额
            //java.math.BigDecimal类来进行精确计算，主要针对商用
